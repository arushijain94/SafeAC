...
def add_delta_var(trajectories, gamma):
    """ Adds td error at all time steps to the trajectory
    ...
    for trajectory in trajectories:
        ...
        rewards = trajectory['rewards']
        deltas_var = rewards**2 - variances + np.append(variances[1:] * (gamma**2), 0) + 2 * gamma * (np.append(values[1:], 0) * rewards)
        trajectory['deltas_var'] = deltas_var

def add_square_disc_sum_rew(trajectories, gamma):
    """ Adds square discounted sum of rewards to all time steps of all trajectories : x_t = \sum_t (\gamma^(t-1) reward_t)
    ...
    for trajectory in trajectories:
        rewards = trajectory['rewards']
        square_disc_sum_rew = np.cumsum([(new_gamma**ind) * val for ind, val in enumerate(rewards)])
        trajectory['square_disc_sum_rew'] = square_disc_sum_rew


def add_gae(trajectories, psi, gamma, lam):
    ...
    """
    for trajectory in trajectories:
        tds = trajectory['deltas']
        initial_value = trajectory['values'][0]
        square_disc_sum_rew = trajectory['square_disc_sum_rew']
        advantages = (1 - 2 * psi * (np.append(0,square_disc_sum_rew[:-1] * gamma) - initial_value)) * discount(tds, gamma * lam)
        trajectory['advantages'] = advantages


# adding advantage function calculator for variance of state
def add_gae_var(trajectories, gamma, lam):
...
def build_train_set(trajectories):
    ...
    disc_sum_rew_square = disc_sum_rew**2
    ...
    return observes, actions, advantages, disc_sum_rew, advantages_var, disc_sum_rew_square
...
def main(env_name, num_episodes, seed, gamma, lam, psi, kl_targ, batch_size, hid1_mult, policy_logvar):
    ...
    save_folder = "../../VarianceReduceResults/MujocoTamar"
    ...
    while episode < num_episodes:
        ...
        add_delta_var(trajectories, gamma)
        add_disc_sum_rew(trajectories, gamma)  # calculated discounted sum of Rs
        add_square_disc_sum_rew(trajectories, gamma)
        add_gae(trajectories, psi, gamma, lam)  # calculate advantage
        add_gae_var(trajectories, gamma, lam)  # calculate advantage for variance
        # concatenate all episodes into single NumPy arrays

        observes, actions, advantages, disc_sum_rew, advantages_var, disc_sum_rew_square = build_train_set(
            trajectories)
        # add various stats to training log:
        log_batch_stats(observes, actions, advantages, disc_sum_rew, advantages_var, disc_sum_rew_square, logger,
                        episode, seed)
        policy.update(observes, actions, advantages, advantages_var, logger)  # update policy
        val_func.fit(observes, disc_sum_rew, logger)  # update value function
        var_func.fit(observes, disc_sum_rew_square, logger)  # update variance function
