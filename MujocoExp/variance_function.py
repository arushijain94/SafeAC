...
class NNVarianceFunction(object):
    ...
    def _build_graph(self):
        ...
        with self.g.as_default():
            self.obs_ph = tf.placeholder(tf.float32, (None, self.obs_dim), 'obs_varfunc')
            self.var_ph = tf.placeholder(tf.float32, (None,), 'val_varfunc')
            ...
            print('Variance Params -- h1: {}, h2: {}, h3: {}, lr: {:.3g}'
            ...
            self.loss = tf.reduce_mean(tf.square(self.out - self.var_ph))  # squared loss
        ...
        for e in range(self.epochs):
            ...
            for j in range(num_batches):
                ...
                feed_dict = {self.obs_ph: x_train[start:end, :],
                             self.var_ph: y_train[start:end]}
                _, l = self.sess.run([self.train_op, self.loss], feed_dict=feed_dict)
        y_hat = self.predict(x)
        loss = np.mean(np.square(y_hat - y))         # explained variance after update
        exp_var = 1 - np.var(y - y_hat) / np.var(y)  # diagnose over-fitting of val func

        logger.log({'VarianceFuncLoss': loss,
