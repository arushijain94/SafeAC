...
class PuddleSimpleEnv(gym.Env):
    ...
    def step(self, action):
        ...
        reward = 0. # reward is given as 0 and later modified in main file is observation belongs to puddle region
        done = np.linalg.norm((self.pos - self.goal), ord=1) < self.goal_threshold
        if done == True:
            reward = 50.0 # reward for the goal state
...
register(
    ...
    timestep_limit=5000, # maximum limit of taking steps is 5000 or either agent reached goal state
)
